...
from py31_16day.demo2.login import login_check
...
class TestLogin(unittest.TestCase):
    excel = Excel(r"C:\project\py31_class\py31_16day\work_15day\cases.xlsx", "login")
    ...
    def test_login(self, item):
        ...
        try:
            ...
            log.error("{}:用例执行失败，失败的信息如下：".format(item['title']))
            # 将异常信息输出到日志，等级为Error
            log.exception(e)
            raise e
        else:
            ...
            self.excel.write_data(row=case_row, column=6, value=str(res))
            log.info("{}:用例测试执行通过！！！".format(item['title']))
